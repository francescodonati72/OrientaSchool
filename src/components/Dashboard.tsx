    ...
    <div className="min-h-screen bg-slate-50">
            ...
            {compA && compB && (
              <>
                ...
                <div className="mb-8 card p-6 border-2 border-dashed border-brand-300 bg-gradient-to-br from-brand-50/50 to-sky-50/50">
                  ...
                  <AIEvalContent aiEval={aiEval} compA={compA} compB={compB} />
                </div>
              </>
            )}
          </>
      ...
      {compA && compB && schoolA && schoolB && (
        <div className="print-only hidden" style={{ fontFamily: 'Inter, sans-serif', color: '#1e293b', padding: '0' }}>
          ...
          {/* PAGINA 1 — Matrice di valutazione */}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            <thead>
              <tr style={{ backgroundColor: '#f1f5f9', borderBottom: '2px solid #334155' }}>
                ...
                <th style={{ textAlign: 'center', padding: '8px 10px', fontWeight: '600', width: '30%' }}>
                  {schoolA.name}{schoolA.location ? ` — ${schoolA.location}` : ''}
                </th>
                <th style={{ textAlign: 'center', padding: '8px 10px', fontWeight: '600', width: '30%' }}>
                  {schoolB.name}{schoolB.location ? ` — ${schoolB.location}` : ''}
                </th>
              </tr>
            </thead>
            <tbody>
            ...
            <div style={{ border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px', backgroundColor: '#fafafa' }}>
              ...
              {aiEval.winner && (
                <div style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '8px', padding: '10px', marginBottom: '12px' }}>
                  <p style={{ fontSize: '12px', fontWeight: '600', color: '#166534', margin: '0' }}>
                ...
                {aiEval.winner && (
                  <div>
                    ...
                    {aiEval.strengthsWinner.map((s, i) => (
                      <p key={i} style={{ fontSize: '11px', color: '#475569', margin: '0 0 3px' }}>• {s}</p>
                    ))}
                  </div>
                )}
                {aiEval.runnerUp && (
...
function AIEvalContent({ aiEval, compA, compB }: { aiEval: any; compA: any; compB: any }) {
  if (!aiEval) return null;
  if (!aiEval.hasEnoughData) {
    return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {aiEval.winner && (
          <div className="rounded-xl bg-white/60 border border-slate-200 p-4">
            ...
            <ul className="space-y-1.5">
              {aiEval.strengthsWinner.map((s: string, i: number) => (
        ...
        {aiEval.runnerUp && (
          <div className="rounded-xl bg-white/60 border border-slate-200 p-4">
            ...
            <ul className="space-y-1.5">
              {aiEval.strengthsRunnerUp.map((s: string, i: number) => (
                <li key={i} className="flex items-start gap-2 text-xs text-slate-600">
